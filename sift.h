...
// Retourn une pair de la valeur maximal et sa position  <MAX,POSITION>
pair<double, int> Max(const vector<double>& a) {

    //Represente 10 � la puissance -8
    double maximum = 1e-8;
    int mxIdx = 0;
...
// Retourn une pair de la valeur minimal inf�rieur � 1e+8 et sa position  <MIN,POSITION>
...
// Retourn une pair de la valeur minimal inf�rieur � 1e+8 et sa position V2 <MIN,POSITION>
...
// Creer la pyramide gaussian (pyramyde d'images flous)
...
//calculer la difference entre les differents couche de la pyramide gaussian
...
//Trouver les extremum du pixel (R,C)
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
                        ...
                        if( abs( contr ) * octaveLayer < 0.03 )
                            break;
                        //cout << abs( contr ) * octaveLayer << endl; // this one
...
void findMatches(const vector<extrema>& ex1, const vector<extrema>& ex2, vector<Point>& matchKP1, vector<Point>& matchKP2, unsigned& cnt_match){
    ...
    for(int i = 0; i < everyDist.size(); i++){
        if(everyMinDist[i]/double(everySecondMinDist[i]) < match_ratio || everyMinDist[i] < 1 ){
            //cout << "(" << ex1[i].pt.x << ", " << ex1[i].pt.y << ") match ("<< ex2[MinIdx[i]].pt.x << ", " << ex2[MinIdx[i]].pt.y << ")" << endl; //this one
